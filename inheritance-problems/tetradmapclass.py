...
class TetradMappingClass(gmc.GeneMappingClass):
	...
	def get_progeny_ascii_table(self) -> str:
		...
		genotypes_col_width = 7 * self.num_genes_int + 5  # 4 characters per genotype plus 5 spaces between
		...
		all_tetrads_tuples = sorted(self.list_of_all_tetrads, reverse=False)
		...
		for tetrad_set_num, tetrad_tuple in enumerate(all_tetrads_tuples, start=1):
			...
			tetrad_count = self.progeny_tetrads_count_dict.get(tetrad_tuple,0)
		...
		if total_tetrad_count != self.progeny_count_int:
		...
		table += separator
		total_str = gml.right_justify_int(self.progeny_count_int, 8)
