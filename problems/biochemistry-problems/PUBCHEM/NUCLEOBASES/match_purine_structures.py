...
"""Blackboard BBQ generator: match purine structures to names.

Each question shows both canonical purines (adenine, guanine) plus one
non-canonical purine (isoguanine or 7-methylguanine) as a distractor.
"""
...
def get_question_text():
	...
	question_text += "<h3>Match the purine structures to their names.</h3>"
...
def write_question(N: int, args) -> str:
	...
	matching_list = nucleobaselib.build_question_names('purine', distractor_name)
...
def parse_arguments():
	parser = bptools.make_arg_parser(
		description="Generate purine structure matching questions (canonical + 1 distractor)."
	)
	parser = bptools.add_choice_args(parser, default=3)
...
def main():
	...
	base_scenarios = list(nucleobaselib.noncanonical_purines)
	if args.scenario_order == 'sorted':
		base_scenarios.sort()
	else:
		random.shuffle(base_scenarios)

	# allow up to args.duplicates questions; cycle through distractor pool
