...
HISTIDINE_STATES = [
	{
		'state_id': 1,
		'smiles': 'C1=C([NH]C=[NH+]1)C[C@@H](C(=O)O)[NH3+]',
		'carboxyl': 'COOH',
		'amino': 'NH<sub>3</sub><sup>+</sup>',
		'imidazole': 'NH and NH<sup>+</sup>',
	},
	{
		'state_id': 2,
		'smiles': 'C1=C([NH]C=[NH+]1)C[C@@H](C(=O)[O-])[NH3+]',
	...
	{
		'state_id': 3,
		'smiles': 'C1=CN=C([NH]1)C[C@@H](C(=O)[O-])[NH3+]',
		'carboxyl': 'COO<sup>-</sup>',
		'amino': 'NH<sub>3</sub><sup>+</sup>',
		'imidazole': 'NH and N',
	},
	{
		'state_id': 4,
		'smiles': 'C1=CN=C([NH]1)C[C@@H](C(=O)[O-])N',
		'carboxyl': 'COO<sup>-</sup>',
		'amino': 'NH<sub>2</sub>',
		'imidazole': 'NH and N',
	},
	{
		'state_id': 5,
		'smiles': 'C1=C([NH]C=[NH+]1)C[C@@H](C(=O)[O-])N',
		'carboxyl': 'COO<sup>-</sup>',
		'amino': 'NH<sub>2</sub>',
		'imidazole': 'NH and NH<sup>+</sup>',
	},
...
def state_for_ph(ph_value: float) -> int:
	"""Return the histidine state id for a given pH value."""
	if ph_value < 1.82:
		return 1
	if ph_value < 6.00:
		return 2
...
def build_question_text(ph_value: float) -> str:
	...
	question_text += "<h3>Histidine Protonation States</h3>"
	question_text += "<p>Histidine has three pKa values: 1.82, 6.00, and 9.17.</p>"
	question_text += (
		"<p><strong>Question:</strong> Which one of the following is the correct "
		f"protonation state of histidine at {ph_emph}?</p>"
	)
...
def build_choice_html(question_num: int, state: dict) -> str:
	...
	details_html = (
		...
		f"Amino: <strong>{state['amino']}</strong><br/>"
		f"Imidazole: <strong>{state['imidazole']}</strong>"
		"</div>"
	)
	choice_html = (
...
def write_question(question_num: int, args) -> str:
	"""Generate one histidine protonation-state question."""
	...
	state_pool = list(HISTIDINE_STATES)
...
#======================================
def parse_arguments():
	parser = bptools.make_arg_parser(description="Generate histidine protonation-state questions.")
